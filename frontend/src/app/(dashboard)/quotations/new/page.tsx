...
export default function NewQuotationPage() {
    ...
    const fetchData = async () => {
      try {
        const [usersData, clientsData] = await Promise.all([
          shoppingApi.listUsers(),
          shoppingApi.listClients()
        ]);
        ...
        if (usersData.length > 0) {
          setQuotationData(prev => ({ ...prev, responsibleId: usersData[0].id }));
        }
      } catch (err) {
  ...
  return (
    <div className="p-8 pb-32">
      <div className="mb-10">
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-4xl text-slate-900 dark:text-white sm:text-4xl">Nova Cotação Profissional</h1>
        <p className="mt-2 text-slate-500 dark:text-petroleum-400 text-slate-500 dark:text-petroleum-400">Configure os dados do processo e adicione os itens para pesquisa.</p>
      </div>
        ...
        <div className="space-y-6 lg:col-span-7">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-petroleum-800 dark:bg-petroleum-900/30">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-petroleum-400 text-slate-500 dark:text-petroleum-400 flex justify-between">
                  Interessado (Cliente)
                  <button 
                    onClick={() => router.push('/clients')}
                    className="text-[10px] text-petroleum-500 hover:underline flex items-center gap-0.5"
                  >
                    <Plus size={10} /> Novo
                  </button>
                </label>
            ...
            <div className="flex gap-2 rounded-2xl bg-slate-50 p-1 mb-6 dark:bg-petroleum-900/50">
              <button
                ...
                className={cn(
                  ...
                  activeTab === 'manual' 
                    ? "bg-white text-slate-900 shadow-sm dark:bg-petroleum-800 dark:text-white shadow-sm dark:bg-petroleum-800 dark:text-white" 
              ...
              <button
                ...
                className={cn(
                  "flex flex-1 items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold transition-all transition-all",
                  activeTab === 'upload' 
                    ? "bg-white text-slate-900 shadow-sm dark:bg-petroleum-800 dark:text-white shadow-sm dark:bg-petroleum-800 dark:text-white" 
                    : "text-slate-500 hover:bg-white/50 dark:text-petroleum-400"
                )}
          ...
          <div className="sticky top-8">
            ...
            {items.length === 0 && (
              <div className="mt-4 flex h-[280px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 p-8 text-center dark:border-petroleum-800 dark:bg-petroleum-900/10">
                <Sparkles size={40} className="text-slate-200 dark:text-petroleum-800 text-slate-200 dark:text-petroleum-800" />
                <p className="mt-4 text-sm text-slate-500 dark:text-petroleum-400 leading-relaxed text-slate-500 dark:text-petroleum-400 leading-relaxed">
